...
def GetNodesEdges(feat_geo, pID):
    ...
    if len(feat_geo) < 2:
        ...
        with arcpy.da.InsertCursor(node_Fc, fields) as cursor:
        ...
        if len(nodes) > 0:
            ...
            if lyr_cnt > 1:
                ...
                arcpy.management.Append(lyr, edge_Fc, "NO_TEST")
...
def backup():
    ...
    # Backup edges
    tmp = arcpy.management.MakeFeatureLayer(edge_Fc, tempLYR, expression)
    cnt = int(arcpy.management.GetCount(tmp)[0])
    if cnt < edges_cnt:
